...
@Component({
  ...
  template: `
    <section class="auth-layout page-enter">
      <div class="auth-context">
        ...
        <div class="auth-points"><span>Plan with phases, modules and lessons</span><span>Import Excel learning plans</span><span>Age-aware learning video safety</span></div>
      </div>
      <mat-card class="auth-card">
        ...
        <form [formGroup]="form" (ngSubmit)="submit()">
          ...
          <mat-form-field appearance="outline">
            ...
            <input matInput type="date" formControlName="dateOfBirth" [max]="latestBirthDate()" autocomplete="bday">
            <mat-hint>Used to apply age-appropriate content safety. Minimum age: {{minimumAge()}}.</mat-hint>
          </mat-form-field>
          <mat-form-field appearance="outline">
        ...
        <p class="privacy-note">Your date of birth is stored with your account and is used to determine age-based safety rules. LearnFlow stores the date, not a fixed age.</p>
  ...
  `,
  styles: [`
    .auth-layout{min-height:calc(100vh - 74px);display:grid;grid-template-columns:1fr minmax(380px,500px);align-items:center;gap:70px;padding:70px max(7vw,28px);background:radial-gradient(circle at 10% 15%,#eef4ff,transparent 38%),#f8fafc;overflow-x:hidden}
    .auth-context{max-width:680px}.auth-context h1{color:#10233f;margin:14px 0 20px}.auth-context>p{font-size:1.08rem;color:#66758a;max-width:620px}.auth-points{display:grid;gap:10px;margin-top:28px;color:#40516a;font-weight:700}.auth-points span:before{content:'✓';color:#16805c;margin-right:9px}.auth-card{padding:34px!important;border-radius:22px!important;border:1px solid #dde4ee!important;box-shadow:0 24px 55px rgba(25,45,75,.11)!important;min-width:0}.auth-card h2{margin:8px 0 6px}form{display:grid;gap:10px;margin-top:22px}mat-form-field{width:100%;min-width:0}.password-toggle{font-size:.76rem;font-weight:800;color:#0c66e4;min-width:56px}.error{color:#bb3f4b}.privacy-note{padding:10px 12px;border-radius:10px;background:#f8fafc;border:1px solid #eaecf0;color:#66758a;font-size:.72rem;line-height:1.5;margin:12px 0 0}.timezone-note,.auth-switch{color:#66758a;margin:15px 0 0}.auth-switch a{color:#0c66e4;font-weight:800;text-decoration:none}.auth-switch a:hover{text-decoration:underline}
    ...
    @media(max-width:480px){.auth-layout{padding:22px 12px}.auth-card{padding:22px!important;border-radius:16px!important}}
  `]
})
export class RegisterComponent {
  ...
  readonly minimumAge = signal(13);
  readonly timezone = Intl.DateTimeFormat().resolvedOptions().timeZone || 'UTC';
  readonly latestBirthDate = computed(() => {
    const date = new Date();
    date.setFullYear(date.getFullYear() - this.minimumAge());
  ...
  constructor(fb: FormBuilder, private readonly auth: AuthService, private readonly router: Router) {
    ...
    this.auth.registrationPolicy().subscribe({
      next: policy => this.minimumAge.set(policy.minimumAge),
      error: () => undefined
    });
  }
  ...
  submit(): void {
    ...
    if (input.dateOfBirth > this.latestBirthDate()) {
      this.error.set(`You must be at least ${this.minimumAge()} years old to create an account.`);
      return;
    }
    this.loading.set(true); this.error.set('');
    this.auth.register({ ...input, timezone: this.timezone }).subscribe({
