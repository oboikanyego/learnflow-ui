          ...
          <div class="issue-section">
            ...
            <div class="comment-composer">
              <mat-form-field appearance="outline">
                ...
                <textarea matInput rows="3" [(ngModel)]="comment" placeholder="Share a note, blocker or learning insight..."></textarea>
              </mat-form-field>
              <button mat-flat-button type="button" (click)="addComment()" [disabled]="!comment.trim() || posting()">
            ...
            <div class="comment-list">
              @for (item of comments(); track item._id) {
                <article class="comment-item">
                  ...
                  <div>
                    <div class="comment-meta"><strong>{{ item.authorName }}</strong><span>{{ item.createdAt | date:'medium' }}</span></div>
                    <p>{{ item.body }}</p>
                  </div>
                </article>
              } @empty {
                <div class="comment-empty">No comments yet. Add the first note for this lesson.</div>
              }
            </div>
          </div>
        </section>
...
export class LessonDetailDialogComponent implements OnInit {
  ...
  constructor(
    readonly dialogRef: MatDialogRef<LessonDetailDialogComponent>,
    @Inject(MAT_DIALOG_DATA) readonly data: { lesson: Lesson }
  ) {}

  ngOnInit(): void { this.loadComments(); }

  loadComments(): void {
    this.api.get<LessonComment[]>(`/api/v1/lessons/${this.data.lesson._id}/comments`).subscribe(value => this.comments.set(value));
  }
  ...
  addComment(): void {
    ...
    this.api.post<{ body: string }, LessonComment>(`/api/v1/lessons/${this.data.lesson._id}/comments`, { body }).subscribe({
      next: item => {
        this.comments.update(items => [...items, item]);
        this.comment = '';
