    ...
    <section class="settings-page page-enter">
      ...
      <form [formGroup]="form" (ngSubmit)="save()">
        <mat-card class="settings-card">
          <mat-card-content>
            <div class="setting-group-head">
              <div><span class="mini-label">Lesson reminders</span><h3>Upcoming learning sessions</h3><p>Control where reminders appear and how early LearnFlow should send them.</p></div>
            </div>

            <div class="setting-row">
              <div><strong>In-app reminders</strong><span>Show an unread LearnFlow notification before scheduled lessons.</span></div>
              <mat-checkbox formControlName="inAppReminders" aria-label="Enable in-app reminders"></mat-checkbox>
            </div>
            <div class="setting-row">
              <div><strong>Email reminders</strong><span>Send a branded reminder email before each scheduled lesson.</span></div>
              <mat-checkbox formControlName="emailReminders" aria-label="Enable email reminders"></mat-checkbox>
            </div>

            <div class="setting-select">
              <div><strong>Remind me before a lesson</strong><span>This timing applies to both enabled reminder channels.</span></div>
        ...
        <mat-card class="settings-card">
          <mat-card-content>
            <div class="setting-group-head">
              <div><span class="mini-label">Email preferences</span><h3>Progress and encouragement</h3><p>Keep the useful messages and switch off the ones you do not need.</p></div>
            </div>

            <div class="setting-row">
              <div><strong>Missed-session emails</strong><span>Receive a gentle email with a direct route back to reschedule a missed lesson.</span></div>
              <mat-checkbox formControlName="missedSessionEmails" aria-label="Enable missed session emails"></mat-checkbox>
            </div>
            <div class="setting-row">
              <div><strong>Celebration emails</strong><span>Receive a confirmation when a new manual, imported, or AI-generated learning plan is ready.</span></div>
              <mat-checkbox formControlName="celebrationEmails" aria-label="Enable celebration emails"></mat-checkbox>
            </div>
          </mat-card-content>
        </mat-card>
        ...
        <div class="settings-note">
          <span>ⓘ</span>
          <p><strong>Account and security emails stay enabled.</strong> Password resets and important account-security messages are transactional and are not controlled by these preferences.</p>
        </div>
        ...
        <div class="settings-actions">
          <button mat-stroked-button type="button" (click)="resetDefaults()" [disabled]="saving()">Restore defaults</button>
          <button mat-flat-button class="primary-cta" type="submit" [disabled]="form.invalid || saving()">{{ saving() ? 'Saving…' : 'Save preferences' }}</button>
        </div>
      </form>
    </section>
  `,
...
export class SettingsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (existing) this.form.setValue(existing);
    else this.auth.loadProfile().subscribe({ next: user => this.form.setValue(user.notificationPreferences ?? DEFAULTS) });
  }
  ...
  save(): void {
    ...
    this.auth.updateNotificationPreferences(this.form.getRawValue()).subscribe({
      next: response => { this.form.setValue(response.notificationPreferences); this.saving.set(false); this.success.set(true); },
