...
interface BillingSettings{provider:Provider;currency:string;proMonthlyPriceMinor:number;proYearlyPriceMinor:number;graceDays:number;enabled:boolean;providerPlanCodes?:{monthly?:string;yearly?:string};updatedAt?:string}
...
<section class="page-enter billing-admin">
  ...
  <div class="grid">
    <article class="panel"><div class="panel-head"><div><span class="mini-label">Runtime configuration</span><h2>Billing settings</h2></div><span class="readiness" [class.ready]="ready()">{{settings.enabled?'Enabled':'Disabled'}}</span></div>
      <div class="form-grid">
        ...
        <div class="toggle-field"><mat-slide-toggle [(ngModel)]="settings.enabled">Enable checkout</mat-slide-toggle><small>Keep disabled until provider credentials, plan mappings and webhook setup are ready.</small></div>
        @if(settings.provider==='PAYSTACK'){
          <mat-form-field appearance="outline"><mat-label>Paystack monthly plan code</mat-label><input matInput [(ngModel)]="settings.providerPlanCodes!.monthly" placeholder="PLN_..."></mat-form-field>
          <mat-form-field appearance="outline"><mat-label>Paystack yearly plan code</mat-label><input matInput [(ngModel)]="settings.providerPlanCodes!.yearly" placeholder="PLN_..."></mat-form-field>
        }
      </div>
      <div class="provider-state"><strong>Provider readiness</strong><span>Selected: {{settings.provider}}</span><span>Checkout switch: {{settings.enabled?'Enabled':'Disabled'}}</span>@if(settings.provider==='PAYSTACK'){<span>Monthly plan: {{settings.providerPlanCodes?.monthly?'Mapped':'Missing'}}</span><span>Yearly plan: {{settings.providerPlanCodes?.yearly?'Mapped':'Missing'}}</span><span>Webhook endpoint: <code>/api/v1/billing/webhooks/paystack</code></span>}<span>Secret credentials remain server-side and are never stored in this settings record.</span></div>
